...
def compute_row(args):
    width, y, x_min, x_max, y_min, y_max, max_iter, height = args
    row = np.empty(width)
...
def generate_mandelbrot_image(width, height, x_min, x_max, y_min, y_max, max_iter):
    args = [(width, y, x_min, x_max, y_min, y_max, max_iter, height) for y in range(height)]
...
def save_mandelbrot(image, elapsed_time):
    ...
    # Save the image
    plt.imshow(image, cmap='inferno', extent=[-2, 1, -1.5, 1.5])
    plt.colorbar()
